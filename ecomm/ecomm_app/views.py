...
from ecomm_app.models import product,cart,orders1
...
def placeorder(request):
   ...
   # print("order is is ",oid)
    for x in c:
        o=orders1.objects.create(order_id=oid,pid=x.pid,uid=x.uid,qty=x.qty)
        o.save()
        x.delete()
        orders=orders1.objects.filter(uid=request.user.id)
        context={}
        context['data']=orders
        np=len(orders)
        s=0
...
def makepayment(request):
  order=orders1.objects.filter(uid=request.user.id)
  s=0
  np=len(order)
  for x in order:
    s=s+x.pid.price*x.qty
    oid=x.order_id  
  client = razorpay.Client(auth=("rzp_test_d36V5PFMdoSPxm", "1GxWdqQe4vHHoqlNNriysuzY"))
...
def sendusermail(request):
  ...
  "Ekart-order placed succesfully",
        msg,
        "sonali.r@itvedant.com",  #two step auth id
        ["sonaliranaware92@gmail.com"],
        fail_silently=False,
    )
